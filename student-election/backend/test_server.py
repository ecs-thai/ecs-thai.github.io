...
class ElectionTests(unittest.TestCase):
 def setUp(self):
  self.tmp=tempfile.TemporaryDirectory();self.root=Path(self.tmp.name);self.db=str(self.root/'vote.sqlite');self.now=round(time.time(),3)
  ...
  initialize(self.db,config);(self.root/'roster.csv').write_text('email\ntest@example.invalid\n');issue(self.db,self.root/'roster.csv',self.root/'codes.csv');self.token=list(csv.DictReader((self.root/'codes.csv').read_text().splitlines()))[0]['voting_code'];self.body={'email':'test@example.invalid','token':self.token,'electionId':config['electionId'],'choices':{o['id']:'ABSTAIN' for o in config['offices']}}
 ...
 def test_wrong_token_and_election(self):
  self.assertEqual(cast(self.db,{**self.body,'token':'000'},self.now+150)[0],403);self.assertEqual(cast(self.db,{**self.body,'electionId':'other'},self.now+150)[0],400)
 ...
 def test_three_character_email_bound_codes(self):
  self.assertEqual(len(self.token),3)
  self.assertEqual(cast(self.db,{**self.body,'email':'other@example.invalid'},self.now+150)[0],403)
  self.assertEqual(cast(self.db,{**self.body,'token':self.token.lower()},self.now+150)[0],200)
 def test_lock_after_five_wrong_attempts(self):
  for _ in range(5):self.assertEqual(cast(self.db,{**self.body,'token':'000'},self.now+150)[0],403)
  self.assertEqual(cast(self.db,self.body,self.now+150)[0],403)
  self.assertEqual(results(self.db,self.now+201)['ballots'],0)
