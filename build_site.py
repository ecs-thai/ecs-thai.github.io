...
def join():
    ...
    body+=f'<section class="section"><div class="wrap prose"><h2>Chulalongkorn University Student Chapter</h2><p>{pair("เตรียมการเลือกตั้งผู้บริหารนักศึกษา 4 ตำแหน่ง ยังไม่มีรายชื่อผู้สมัครและกำหนดการลงคะแนน สามารถทดลองบัตรเลือกตั้งได้", "Preparing elections for four student officer positions. Candidates and voting dates have not been announced. A sample ballot is available.")}</p>{link("student-election/","การเลือกตั้ง Student Chapter →","Student Chapter election →","button")}</div></section>'
    body+=f'<section class="section"><div class="wrap prose"><h2>ECS Thailand Section</h2><p>{pair("เตรียมการเสนอชื่อและรับประวัติผู้สมัครคณะกรรมการบริหาร ยังไม่เปิดรับข้อมูลจริง", "Preparing nominations and candidate profiles for the Executive Committee. Submissions are not yet open.")}</p>{link("section-election/","เสนอชื่อคณะกรรมการบริหาร Section →","Section Executive Committee nominations →","button")}</div></section>'
