    ...
    try:
        ...
        for o in config['offices']:
            if o.get('type')=='APPROVAL':
                values=votes[o['id']]
                if not isinstance(values,dict) or set(values)!={c['id'] for c in o['candidates']} or any(v not in ['YES','NO','ABSTAIN'] for v in values.values()):return 400,{'error':'invalid_ballot'}
                continue
...
def results(path,now=None):
    ...
    try:
        ...
        counts={o['id']:({c['id']:{'YES':0,'NO':0,'ABSTAIN':0} for c in o['candidates']} if o.get('type')=='APPROVAL' else {c['id']:0 for c in o['candidates']}|{'ABSTAIN':0}) for o in config['offices']}
        rows=db.execute('SELECT choices FROM ballots').fetchall()
        for row in rows:
            for office,candidate in json.loads(row[0]).items():
                if isinstance(candidate,dict):
                    for ident,value in candidate.items():counts[office][ident][value]+=1
                else:counts[office][candidate]+=1
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if not self.allowed():return self.reply(403,{'error':'origin'})
        if self.path not in ['/ballots','/candidate/profile','/candidate/submit','/nominations']:return self.reply(404,{'error':'not_found'})
        ...
        try:
            ...
            elif self.path=='/nominations':
                from nominations import receive
                status,data=receive(self.server.dbpath,body)
            else:
