...
def schema(db):
    db.execute('CREATE TABLE IF NOT EXISTS candidates(id TEXT PRIMARY KEY, office TEXT NOT NULL, invite_hash TEXT UNIQUE NOT NULL, expires REAL NOT NULL, profile TEXT NOT NULL, photo BLOB, submitted INTEGER NOT NULL DEFAULT 0, approved INTEGER NOT NULL DEFAULT 0, shortlisted INTEGER NOT NULL DEFAULT 0)')
    ...
    with open(output,'x',encoding='utf-8',newline='') as f:
        ...
        try:
            ...
            for r in rows:
                ...
                db.execute('INSERT INTO candidates(id,office,invite_hash,expires,profile) VALUES(?,?,?,?,?)',(ident,r.get('office',''),digest(secret),time.time()+days*86400,json.dumps({'name':r.get('name',''),'affiliation':'','bio':''})))
...
def candidate_request(path,route,body):
    ...
    try:
        ...
        db.execute('UPDATE candidates SET office=?,profile=?,photo=?,submitted=1,approved=0,shortlisted=0 WHERE id=?',(office,json.dumps(profile),photo,row[0]));db.execute('COMMIT')
...
def export_profiles(path,out,approved=False):
    ...
    rows=db.execute('SELECT id,office,profile,photo,approved,shortlisted FROM candidates WHERE submitted=1'+(' AND approved=1' if approved else '')).fetchall();profiles=[]
    for ident,office,profile,photo,ok,shortlisted in rows:
        filename=ident+'.jpg';(target/filename).write_bytes(photo);profiles.append({'id':ident,'office':office,**json.loads(profile),'photo':filename,'approved':bool(ok),'shortlisted':bool(shortlisted)})
    (target/'profiles.json').write_text(json.dumps(profiles,ensure_ascii=False,indent=2));db.close()

def shortlist(path,ident):
    db=connect(path);schema(db);config=config_for(db)
    try:
        if config.get('opensAt') and time.time()>=stamp(config['opensAt']):raise ValueError('Election has opened')
        if db.execute('UPDATE candidates SET shortlisted=1 WHERE id=? AND submitted=1',(ident,)).rowcount!=1:raise ValueError('Candidate must submit profile and publication consent first')
    finally:db.close()
...
def approve(path,ident):
    ...
    if db.execute('UPDATE candidates SET approved=1 WHERE id=? AND submitted=1 AND shortlisted=1',(ident,)).rowcount!=1:raise ValueError('Candidate must first pass Nominating Committee review')
    db.close()
...
def main():
    ...
    i=sub.add_parser('invite');i.add_argument('--roster',required=True);i.add_argument('--output',required=True);i.add_argument('--url',default='https://ecs-thai.github.io/section-election/candidate.html');i.add_argument('--days',type=int,default=14)
    ...
    n=sub.add_parser('shortlist');n.add_argument('--id',required=True);n.add_argument('--eligibility-and-consent-checked',action='store_true',required=True)
    ...
    elif args.command=='shortlist':shortlist(args.db,args.id)
