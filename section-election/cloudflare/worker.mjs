...
export default {async fetch(request,env){
 ...
 try{
  ...
  if(method==='GET'&&path==='/approved'){
   ...
   return reply({electionId:config.electionId,title:'ECS Thailand Section · Executive Committee Election',offices:offices.map(o=>({...o,title:o.en,candidates:results.filter(c=>c.office===o.id).map(c=>({id:c.id,...JSON.parse(c.profile),photo:url.origin+'/photos/'+c.id}))}))});
  }
  ...
  if(admin){
   ...
   if(!['/nominations','/candidate/profile','/candidate/submit'].includes(path))fail('not_found',404);
  ...
  if(admin){
   if(path==='/admin/intake'){if(typeof b.open!=='boolean')fail('invalid_request');await query(env,'UPDATE settings SET nominations_open=? WHERE id=1',+b.open).run();return reply({ok:true})}
   ...
   if(path==='/admin/invite'){
    ...
    return reply({id,upload_link:'https://ecs-thai.github.io/section-election/candidate.html#invite='+token});
   }
   ...
   if(path==='/admin/shortlist'||path==='/admin/approve'){
    ...
    const isApprove=path.endsWith('/approve');if(isApprove&&c.shortlisted_revision!==c.revision)fail('review_required',409);
